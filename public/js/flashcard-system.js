...
function getDefaultFlashcards() {
    return [
        {
            front: "Benvenuto nelle flashcards di Diritto Privato!",
            back: "Clicca su una modalità di studio per iniziare. Il sistema utilizzerà l'algoritmo di ripetizione spaziata per aiutarti a memorizzare meglio le informazioni.",
            interval: 0,
            reviews: 0,
            ease: 2.5,
            dueDate: null,
...
function enhanceFlashcardStructure() {
    ...
    if (allCards.length === 0) {
        console.warn('⚠️ Nessuna flashcard disponibile, carico quelle di default');
        dirittoPrivatoFlashcards = getDefaultFlashcards().map(normalizeCard);
    }
...
function resetFlashcardsProgress() {
    ...
    let baseCards = originalFlashcardsSnapshot.length
        ...
        : (window.dirittoPrivatoFlashcards && window.dirittoPrivatoFlashcards.length
            ...
            : getDefaultFlashcards().map(normalizeCard));
...
function loadDirittoPrivatoFlashcards(callback) {
    ...
    script.onload = function () {
        ...
        if (window.dirittoPrivatoFlashcards && window.dirittoPrivatoFlashcards.length > 0) {
            ...
            console.warn("⚠️ Nessuna flashcard trovata in window.dirittoPrivatoFlashcards, uso default");
            dirittoPrivatoFlashcards = getDefaultFlashcards().map(normalizeCard);
            captureOriginalSnapshot(dirittoPrivatoFlashcards);
        }
    ...
    script.onerror = function () {
        ...
        console.log('🔄 Provo a caricare le flashcards di default...');
        dirittoPrivatoFlashcards = getDefaultFlashcards().map(normalizeCard);
        captureOriginalSnapshot(dirittoPrivatoFlashcards);
        enhanceFlashcardStructure();
